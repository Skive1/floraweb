            ...
            if (session != null) {
                ...
                if ("Delivery".equals(role) && amountStr != null) {
                    ...
                    if (result) {
                        ...
                        out.print("{\"success\": true, \"message\": \"Rút tiền thành công!\", \"newBalance\": " + (currentBalance - amount) + "}");
                        out.flush();
                    } else {
                        // Nếu không cập nhật được số dư
                        response.setContentType("application/json");
                        PrintWriter out = response.getWriter();
                        out.print("{\"success\": false, \"message\": \"Rút tiền thất bại.\"}");
                        out.flush();
                    }
                } else {
                    response.sendRedirect(MyAppConstants.WithDrawFeatures.ERROR);
                }
            }
