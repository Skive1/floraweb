...
public class CancelOrderServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        int eventOrderId = Integer.parseInt(request.getParameter("eventOrderId"));
        try (PrintWriter out = response.getWriter()) {
            EventOrderDAO dao = new EventOrderDAO();
            boolean result = dao.userCancelOrder(eventOrderId);
            if (result) {
            ...
            log("cancelUserOrder _SQL_ " + ex.getMessage());
        } catch (NamingException ex) {
            log("cancelUserOrder _Naming_ " + ex.getMessage());
        } finally {
